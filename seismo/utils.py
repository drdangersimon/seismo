...
# decorators 
class force_double_precision(object):
    def __init__(self, open_cl_period):
        self._open_cl_period = open_cl_period

    def __call__(self, *args, **kwargs):
        # check what kind of error is throwns
        try:
            return self._open_cl_period(*args, **kwargs)
        except :
            pass

    def __getattr__(self, name):
        ''' '''
        #return getattr(self._open_cl_period, name)
class Matrix_mul_gpu(object):
    def __init__(self):
        '''initalizes matrix object for later use'''
        self.Matrix_KERNEL_CODE = """
// Thread block size
...
matrixMul( __global double* C, __global double* A, __global double* B)
{
    ...
    C[get_global_id(1) * get_global_size(0) + get_global_id(0)] = Csub;
        ...
        # check if actual device has memory
    ...
    def __call__(self, a, b):
        ...
        See for numpy.dot for more info.'''
        # check matrix sizes
        # (height, width)
        if not a.shape[1] == b.shape[0]:
            raise ValueError
        if not len(a.shape) == 2 or not len(a.shape) == 2:
            raise ValueError('Input matrix not correct shape. 2-D arrays only.')
        # make out array
        out = np.empty((a.shape[0], b.shape[1])) #.astype(np.float32)
        # set up gpu kernel stuff
        kernel_params = {"block_size": self.block_size,
                "w_a":a.shape[1], "h_a":a.shape[0], "w_b":b.shape[1]}

        prg = cl.Program(self.ctx, self.Matrix_KERNEL_CODE % kernel_params,
                ).build(options=self.options)
        ...
        out_buf = cl.Buffer(self.ctx, mf.WRITE_ONLY, size=out.nbytes)
        # do matrix multiplication
        event = kernel(self.queue, out.shape[::-1], (self.block_size, self.block_size),
                    out_buf, a_buf, b_buf)
        event.wait()
        # get data from GPU
...
if __name__ == '__main__':
    ...
    if "NVIDIA" in queue.device.vendor:
        ...
        options = ""
    prg = cl.Program(ctx, Matrix_KERNEL_CODE % kernel_params,
